...
2. 제조사(${brand || "해당 제조사"}) 공식 권장 표준 교체 주기와 사용자의 환경 조건(${condition === "harsh" ? "가혹 조건" : "일반 조건"})을 반영하여 최적의 '다음 교체 예정일'과 '남은 수명 백분율(%)'을 산출하세요.
3. 교체 시기를 놓쳤을 때 발생할 수 있는 위험 요소(자산 손상, 성능 저하, 위생 문제 등)와 실용적인 정비/관리 팁을 제공하세요.
4. 반드시 순수한 JSON 형식만 응답하세요. 마크다운(\`\`\`json 등) 코드블록 없이 순수 JSON 객체만 반환해야 합니다.
